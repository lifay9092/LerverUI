...
sealed class TreeConfig<T : Any, P : Any> {
    ...
    abstract var imgCall: ((TreeItem<T>) -> Node)?
    ...
    class IdParentIdConfig<T : Any, P : Any>(
        ...
        override var imgCall: ((TreeItem<T>) -> Node)? = null,
    ) : TreeConfig<T, P>() {
...
class LerverTreeTempNode<T : Any>(
    var nodeValue: T,
    var nodeChildren: List<LerverTreeTempNode<T>>?,
...
// 封装的智能树组件
class LerverTreeView<T : Any, P : Any>(
//    private val config: TreeConfig<T, P>
) : TreeView<T>() {

    private lateinit var TREE_CONFIG: TreeConfig<T, P>
    val treeId = this.hashCode().toString()

    //root下的节点 原始树结构缓存
    private var ORIGINAL_ROOT_NODE: LerverTreeTempNode<T>? = null

    //过滤状态
    private var IS_FILTERED = false

    // T实例.code -> 原始节点数据实例
    private val ID_TO_VALUE_NODE = HashMap<String, LerverTreeTempNode<T>>()
...
//    private val codeToExpandedStatus = HashMap<Int, Boolean>()
    ...
    fun LoadTreeConfig(config: TreeConfig<T, P>) {
        TREE_CONFIG = config
    }
...
//        println("root.code():rootHashCode")
    ...
    // 保存树结构到缓存
    private fun saveTreeToCache(rootId: String?) {
        val rootIdTemp = rootId ?: getIdByValue(root.value)
        ORIGINAL_ROOT_NODE = LerverTreeTempNode(
            root.value,
            root.children?.map { itemToNode(it) })
        ID_TO_VALUE_NODE[rootIdTemp] = ORIGINAL_ROOT_NODE!!
     ...
     * 树项节点转成缓存节点
     */
    private fun itemToNode(treeItem: TreeItem<T>): LerverTreeTempNode<T> {
//        treeItem.treeViewddd = this@SmartTreeView

        val tempNode =
            LerverTreeTempNode(treeItem.value, treeItem.children?.map { itemToNode(it) })
    ...
    private fun buildTreeByIdParentId() {
        ...
        val rootId = TREE_CONFIG.idProperty(root.value)
        val childrenList = TREE_CONFIG.loadChildren(root.value)
        childrenList?.filter {
            rootId == TREE_CONFIG.parentIdProperty(it)
        }?.forEach { data ->
    ...
    private fun createTreeItemByIdParentId(data: T, list: List<T>?): TreeItem<T> {
        ...
        val itemId = TREE_CONFIG.idProperty(data)
        list?.filter {
            itemId == TREE_CONFIG.parentIdProperty(it)
        }?.forEach { child ->
    ...
    private fun createTreeItemByChildren(data: T): TreeItem<T> {
        val treeItem = createTreeItemBase(data)
        TREE_CONFIG.childrenProperty(data)?.let { children ->
    ...
    private fun buildLazyTree() {
        this.setOnMouseClicked {
            ...
            if (item != null && it.clickCount == 2 && !item.isExpanded && item.children.isEmpty()) {
                val childrenValue = TREE_CONFIG.loadChildren(item.value)
        ...
        root.children.setAll(TREE_CONFIG.loadChildren(root.value)?.map { createTreeItemBase(it) } ?: emptyList())
    }

    private fun createTreeItemBase(data: T): TreeItem<T> {
        val treeItem = if (TREE_CONFIG.isCheckBox) CheckBoxTreeItem<T>(data) else TreeItem<T>(data)
        treeItem.treeView = this@LerverTreeView
        TREE_CONFIG.imgCall?.let { it(treeItem) }.let {
            treeItem.graphic = it
        }
        return treeItem
    }
    ...
    private fun addNode(parentItem: TreeItem<T>, data: T) {
        ...
        val treeItem = createTreeItemBase(data)
        val lerverTreeTempNode = LerverTreeTempNode(data)
        lerverTreeTempNode.expanded.apply {
            unbind()
        ...
        ID_TO_VALUE_NODE[getIdByValue(parentItem.value)]!!.nodeChildren.let {
            (it as MutableList<LerverTreeTempNode<T>>?)?.add(lerverTreeTempNode)
        }
        //节点缓存
        ID_TO_TREE_ITEM[id] = treeItem
        ID_TO_VALUE_NODE[id] = lerverTreeTempNode
        CODE_TO_TREEITEM_MAP[treeItem.hashCode()] = treeItem
    }
...
//        }
    ...
    @Synchronized
    fun RefreshTree() {
        LoadTree(TREE_CONFIG.loadChildren)
    }

    // 过滤树 节点或子节点满足条件都显示
    @Synchronized
    fun FilterTree(predicate: (T) -> Boolean) {
        IS_FILTERED = true
        loadCacheToTree(predicate)
    }

    // 递归修剪树结构的核心方法
    private fun pruneTree(node: LerverTreeTempNode<T>, predicate: ((T) -> Boolean)?): Boolean {
    ...
    // 从缓存还原树
    private fun loadCacheToTree(predicate: ((T) -> Boolean)? = null) {
        root.children.clear()
     ...
     * 缓存节点转成树项节点
     */
    private fun nodeToItem(item: LerverTreeTempNode<T>): TreeItem<T> {
    ...
    private fun getIdByValue(item: T): String {
        val code = TREE_CONFIG.idProperty.invoke(item).toString()
        return code
    }

    fun GetItemByBusiId(id: P): TreeItem<T>? {
        return ID_TO_TREE_ITEM[id.toString()]
    }
}
